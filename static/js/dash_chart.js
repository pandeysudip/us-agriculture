...
function init() {
    ...
    d3.json('/data/crops').then(function (data) {
        ...
        //var Sortedstates = uniqueState.sort(function(a, b){return a-b});
        uniqueState.sort(function (a, b) {
            if (a < b)
                return -1;
            if (a > b)
                return 1;
...
function CreateallCharts(dataselected) {
    ...
    var xValues = AllStates;
    var yValues = TotalSales;
    var barColors = ["red", "green", "blue", "orange", "brown", "red", "green", "blue", "orange", "brown"];

    //d3.select("bar").selectAll("canvas").remove();
    //var menubar = d3.select("#bar");
    //menubar.append("canvas").attr("id", "bar1").style("width:100%;max-width:1000px")


    chart.destroy()
    chart = new Chart("bar1", {
        type: "bar",
        data: {
            labels: AllStates,
            datasets: [{
...
function GraphsInfo() {
    ...
    if (typeoption === "Field Crops") {
        ...
        panel.append("h3").text("Total Sales per State:" + "$" + acumulator);
        panel.append("h3").text("Average Sales per County:" + "$" + mean);
        CreateallCharts(vegetablesdata)
    }
